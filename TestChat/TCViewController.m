...
- (void)_reconnect;
{
    ...
    _webSocket = [[SRWebSocket alloc] initWithURLRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"ws://192.168.5.183:3000/"]]];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField{
    ...
    if (textField.text && textField.text.length>0) {
        ...
        if (names && [names count]>=2) {
            self.userName =[names objectAtIndex:0];
            self.targetName=[names objectAtIndex:1];
        }
...
- (void)webSocketDidOpen:(SRWebSocket *)webSocket;
{
    ...
    NSDictionary *messageDictionary =[NSDictionary dictionaryWithObjectsAndKeys:self.userName,@"login",nil];
    NSString *messageString =[messageDictionary JSONString];
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text;
{
    if ([text rangeOfString:@"\n"].location != NSNotFound) {
        ...
        NSDictionary *messageDictionary =[NSDictionary dictionaryWithObjectsAndKeys:self.userName,@"username",self.targetName,@"targetName", message,@"msg",nil];
        NSString *messageString =[messageDictionary JSONString];
        [_webSocket send:messageString];
